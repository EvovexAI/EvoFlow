...
test('PROVIDER_PRESETS contains Moonshot / Kimi entry', () => {
  const moonshot = PROVIDER_PRESETS.find(p => p.key === 'moonshot')
  assert.ok(moonshot, 'Moonshot provider preset should exist')
  assert.equal(moonshot.label, '月之暗面 Kimi')
  assert.equal(moonshot.api, 'openai-completions')
  assert.equal(moonshot.baseUrl, 'https://api.moonshot.cn/v1')
})

test('Anthropic preset uses official SDK base (no trailing /v1)', () => {
  const anthropic = PROVIDER_PRESETS.find(p => p.key === 'anthropic')
  assert.ok(anthropic, 'Anthropic provider preset should exist')
  assert.equal(anthropic.api, 'anthropic-messages')
  // Official docs: base https://api.anthropic.com ; endpoint POST /v1/messages
  assert.equal(anthropic.baseUrl, 'https://api.anthropic.com')
  assert.ok(!anthropic.baseUrl.endsWith('/v1'), 'SDK base must not include /v1')
})

test('MODEL_PRESETS contains Moonshot Kimi models', () => {
  assert.ok(MODEL_PRESETS.moonshot, 'MODEL_PRESETS should have a moonshot key')
  const ids = MODEL_PRESETS.moonshot.map(m => m.id)
