...
import { prepareForceGraphData } from "../lib/knowledge-graph-layout.js";
...
// ForceGraph 首屏很重：与 owned 页对齐，按需 lazy
...
function LazyGraph(props) {
  return (
    <React.Suspense
      fallback={
        <div className="kv-graph-lazy-fallback" style={{ padding: 24, textAlign: "center", opacity: 0.6 }}>
          图谱加载中…
        </div>
      }
    >
      <LazyKnowledgeForceGraph {...props} />
    </React.Suspense>
  );
}
...
function GraphExplorer({
    ...
    return (
      <div className="kv-graph-explorer">
        ...
        <div className="kv-graph-explorer__canvas">
          <LazyGraph {...graphProps} mode="compact" showToolbar={false} showDetailPanel={false} />
        </div>
        {expanded ? (
  ...
  return (
    <div className="kv-graph-explorer is-large kv-knowledge-shell">
      ...
      />
      <div className="kv-knowledge-shell__graph">
        <LazyGraph {...graphProps} mode="large" />
      </div>
    </div>
  );
}
...
export default function KnowledgeVaultsPage() {
  ...
  const loadVaultsGenRef = useRef(0);
  ...
  async function loadVaults() {
    ...
    try {
      ...
      // Paint light list first; status enrich patches in place (listSort* freezes order).
      const light = items.map((vault) =>
        normalizeVault({
          ...vault,
      ...
      const gen = ++loadVaultsGenRef.current;
      void Promise.all([
        rolesPromise,
        Promise.all(
        ...
        if (gen !== loadVaultsGenRef.current) return;
        setVaults(enriched);
      });
    } catch (error) {
