...
export async function render() {
  ...
  await refresh()
  if (boot.tab === 'runs') {
    await renderRunsPanel(page, boot.date, boot.task)
  }
  try {
...
async function renderRunsPanel(page, dateKey = '7d', filterTaskId = '') {
  ...
  _runsRowCache = {}
  try {
    const list = Array.isArray(_tasks) ? _tasks.slice(0, 80) : []
    await Promise.all(
      list.map(async (t) => {
        try {
          const res = await api.automationHistory(t.id, 20)
          const runs = Array.isArray(res?.runs) ? res.runs : []
          for (const r of runs) {
            const ts = Date.parse(String(r.started_at || r.created_at || r.finished_at || ''))
            const rowId = `${t.id}-${r.run_id || r.id || ts}`
          ...
          /* ignore single-task history errors */
        }
      }),
    )
  } catch (e) {
