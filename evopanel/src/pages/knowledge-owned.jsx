...
export default function KnowledgeOwnedPage() {
  ...
  const loadDocs = useCallback(async (kbId) => {
    ...
    try {
      const res = await api.listOwnedKnowledgeDocuments(kbId);
      setDocs(res.items || []);
      setFolders(res.folders || []);
      try {
        const tagsRes = await api.listOwnedKnowledgeTags(kbId);
        setKbTags(tagsRes.items || []);
      } catch {
        setKbTags([]);
      }
      try {
        const jobsRes = await api.listOwnedKnowledgeJobs(kbId, { limit: 20 });
