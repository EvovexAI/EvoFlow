...
export default {
  title: _('通信与自动化', 'Communication & Automation', '通信與自動化'),
  desc: _('管理 AI 在各消息渠道中的行为方式：如何回复消息、支持哪些命令、如何接收外部通知等', 'Manage how AI behaves in messaging channels: reply style, commands, external notifications, etc.', '管理 AI 在各訊息頻道中的行為方式：如何回覆訊息、支援哪些命令、如何接收外部通知等'),
  tabMessages: _('消息', 'Messages', '訊息'),
  tabBroadcast: _('广播', 'Broadcast', '廣播'),
  tabCommands: _('命令', 'Commands', '命令'),
  ...
  replyPrefix: _('回复前缀', 'Reply Prefix', '回覆前綴'),
  replyPrefixPlaceholder: _('如 [{model}] 或 auto', 'e.g. [{model}] or auto', '如 [{model}] 或 auto'),
  replyPrefixHint: _('每条 AI 回复开头自动加的前缀。支持 {model}、{provider}、{thinkingLevel} 等变量。设为 auto 则显示 Agent 名称', 'Auto-prepended prefix for each AI reply. Supports {model}, {provider}, {thinkingLevel} variables. Set to auto for Agent name.', '每條 AI 回覆開頭自動加的前綴。支援 {model}、{provider}、{thinkingLevel} 等變數。設為 auto 則顯示 Agent 名稱'),
  ...
  removeAckAfterReply: _('回复后移除确认反应', 'Remove ack after reply', '回覆後移除確認反應'),
  removeAckAfterReplyHint: _('回复发送成功后自动删除之前的确认 emoji', 'Auto-remove the ack emoji after the reply is sent', '回覆發送成功後自動刪除之前的確認 emoji'),
  ...
  debounceMsHint: _('合并快速连续消息的等待时间（毫秒），避免 AI 对每条消息逐一回复', 'Wait time to merge rapid consecutive messages (ms), prevents AI from replying to each one', '合併快速連續訊息的等待時間（毫秒），避免 AI 對每條訊息逐一回覆'),
  ...
  broadcastParallel: _('并行（parallel）— 同时发送给所有目标', 'Parallel — send to all targets simultaneously', '並行（parallel）— 同時發送給所有目標'),
  broadcastSequential: _('顺序（sequential）— 逐个发送，严格有序', 'Sequential — send one by one, strictly ordered', '順序（sequential）— 逐個發送，嚴格有序'),
  broadcastHint: _('当消息需要广播给多个 Agent 时的处理策略。并行更快，顺序更可控', 'Strategy for broadcasting to multiple Agents. Parallel is faster, sequential is more controlled.', '當訊息需要廣播給多個 Agent 時的處理策略。並行更快，順序更可控'),
  slashCommands: _('斜杠命令', 'Slash Commands', '斜線命令'),
  cmdText: _('文本命令解析', 'Text Command Parsing', '文字命令解析'),
  cmdTextHint: _('允许通过 / 前缀在聊天中执行命令', 'Allow executing commands via / prefix in chat', '允許透過 / 前綴在聊天中執行命令'),
  cmdBash: _('Bash 命令', 'Bash Commands', 'Bash 命令'),
  cmdBashHint: _('允许用 ! 前缀或 /bash 在聊天中执行 Shell 命令（危险）', 'Allow ! prefix or /bash to execute shell commands in chat (dangerous)', '允許用 ! 前綴或 /bash 在聊天中執行 Shell 命令（危險）'),
  cmdConfig: _('/config 命令', '/config Command', '/config 命令'),
  cmdConfigHint: _('允许在聊天中查看/修改配置', 'Allow viewing/modifying config in chat', '允許在聊天中查看/修改設定'),
  cmdDebug: _('/debug 命令', '/debug Command', '/debug 命令'),
  cmdDebugHint: _('允许在聊天中查看调试信息', 'Allow viewing debug info in chat', '允許在聊天中查看除錯資訊'),
  cmdRestart: _('重启命令', 'Restart Command', '重啟命令'),
  cmdRestartHint: _('允许通过命令重启 Gateway', 'Allow restarting Gateway via command', '允許透過命令重啟 Gateway'),
  nativeCommands: _('原生命令注册', 'Native Command Registration', '原生命令註冊'),
  nativeLabel: _('原生命令', 'Native Commands', '原生命令'),
  nativeAuto: _('自动', 'Auto', '自動'),
  nativeEnabled: _('启用', 'Enabled', '啟用'),
  nativeDisabled: _('禁用', 'Disabled', '停用'),
  nativeHint: _('在支持的渠道（Telegram、Discord）自动注册原生命令菜单', 'Auto-register native command menus in supported channels (Telegram, Discord)', '在支援的頻道（Telegram、Discord）自動註冊原生命令選單'),
  webhookSettings: _('Webhook 设置', 'Webhook Settings', 'Webhook 設定'),
  webhookEnabled: _('启用 Webhook', 'Enable Webhook', '啟用 Webhook'),
  webhookEnabledHint: _('允许外部服务通过 HTTP 触发 AI 执行', 'Allow external services to trigger AI execution via HTTP', '允許外部服務透過 HTTP 觸發 AI 執行'),
  ...
  webhookTokenHint: _('外部请求需在 Header 中携带此 Token 才能触发 Webhook', 'External requests must include this token in header to trigger Webhook', '外部請求需在 Header 中攜帶此 Token 才能觸發 Webhook'),
  webhookSessionKey: _('默认 Session Key', 'Default Session Key', '預設 Session Key'),
  webhookSessionKeyHint: _('Webhook 触发的 Agent 会话标识。留空则每次自动生成', 'Session identifier for Webhook-triggered Agent. Leave empty for auto-generated.', 'Webhook 觸發的 Agent 對話標識。留空則每次自動產生'),
  webhookMaxBody: _('请求体大小限制（字节）', 'Max Body Size (bytes)', '請求體大小限制（位元組）'),
  noLimit: _('默认无限制', 'No limit by default', '預設無限制'),
