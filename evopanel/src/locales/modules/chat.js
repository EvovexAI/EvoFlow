...
export default {
  ...
  confirmDeleteSession: _('确定删除会话「{label}」？', 'Delete session "{label}"?', '確定刪除對話「{label}」？'),
  sessionDeleted: _('会话已删除', 'Session deleted', '對話已刪除'),
  deleteFailed: _('删除失败: ', 'Delete failed: ', '刪除失敗: '),
  confirmResetSession: _('确定要重置会话「{label}」吗？\n\n重置后将清空该会话的所有聊天记录，此操作不可撤销。', 'Reset session "{label}"?\n\nThis will clear all chat history for this session. This action cannot be undone.', '確定要重置對話「{label}」嗎？\n\n重置後將清空該對話的所有聊天紀錄，此操作不可撤銷。'),
  ...
  gatewayNotReadySend: _('服务未就绪，连接成功后再发送', 'Service not ready, will send after connection', '服務未就緒，連線成功後再發送'),
  ...
  connectionRejected: _('连接被 Gateway 拒绝，请点击「修复并重连」', 'Connection rejected by Gateway, click "Fix & Reconnect"', '連線被 Gateway 拒絕，請點擊「修復並重連」'),
  noMessages: _('还没有消息，开始聊天吧', 'No messages yet, start chatting!', '還沒有訊息，開始聊天吧', 'メッセージなし', '메시지 없음', 'Không có tin nhắn', 'Sin mensajes', 'Sem mensagens', 'Нет сообщений', 'Aucun message', 'Keine Nachrichten'),
  imageHistoryHint: _('部分历史图片无法显示（Gateway 不保留图片原始数据，仅当前会话内可见）', 'Some history images cannot be displayed (Gateway does not retain original image data, only visible in current session)', '部分歷史圖片無法顯示（Gateway 不保留圖片原始資料，僅目前對話內可見）'),
  ...
  tool: _('工具', 'Tool'),
  file: _('文件', 'File', '檔案'),
  compacting: _('正在整理对话记录…', 'Organizing conversation…'),
  remaining: _('剩余', 'Remaining', '剩餘'),
  connectFailed: _('连接失败', 'Connection failed', '連線失敗'),
  fixDoneReconnecting: _('修复完成，正在重连...', 'Fix done, reconnecting...', '修復完成，正在重連...'),
  fixing: _('修复中...', 'Fixing...'),
  writingConfig: _('正在写入配置...', 'Writing config...', '正在寫入設定...'),
  ...
  gatewayNotReady: _('服务连接未就绪', 'Service connection not ready', '服務連線未就緒'),
  connectingGateway: _('正在连接服务…', 'Connecting to service…', '正在連線服務…'),
  fixAndReconnect: _('修复并重连', 'Fix & Reconnect', '修復並重連'),
  gatewaySettings: _('连接设置', 'Connection Settings', '連線設定'),
  firstUseHint: _('首次使用？请确保服务已启动，或点击「修复并重连」自动修复配置', 'First time? Make sure the service is running, or click "Fix & Reconnect" to auto-fix', '首次使用？請確保服務已啟動，或點擊「修復並重連」自動修復設定'),
  guideTitle: _('你正在使用「实时聊天」', 'You are using Live Chat', '你正在使用「實時聊天」'),
  guideDesc: _('此页面连接 AI 助手，对话由你部署的服务处理。', 'This page connects to your AI assistant. Conversations are processed by your deployed service.', '此頁面連線 AI 助手，對話由你部署的服務處理。'),
  guideHint: _('如需使用内置 AI 助手（与「实时对话」相互独立），请前往左侧菜单「AI 助手」页面。', 'To use the built-in AI Assistant (separate from Live Chat), go to "AI Assistant" in the left menu.', '如需使用內置 AI 助手（與「實時聊天」相互獨立），請前往左側選單「AI 助手」頁面。'),
  ...
  hostedAgent: _('目标模式', 'Goal Mode', '目標模式'),
  hostedBadge: _('目标', 'Goal', '目標'),
  taskGoal: _('任务目标', 'Task Goal', '任務目標'),
  taskGoalPlaceholder: _('例如：持续优化此仓库代码质量，直到没有可改进的地方', 'e.g. Continuously optimize code quality until no improvements remain', '例如：持續最佳化此倉庫代碼品質，直到沒有可改進的地方'),
  hostedHint: _('目标模式会持续在本应用中推进此目标。模型使用 <a href="#/assistant" class="hosted-agent-link">AI 助手</a> 的配置。', 'Goal mode keeps working toward this goal in the app. Uses <a href="#/assistant" class="hosted-agent-link">AI Assistant</a> model config.', '目標模式會持續在本應用中推進此目標。模型使用 <a href="#/assistant" class="hosted-agent-link">AI 助手</a> 的設定。'),
  maxReplies: _('最大回复次数', 'Max Replies', '最大回覆次數'),
  timerAutoStop: _('定时自动停止', 'Auto-stop Timer', '定時自動停止'),
  startHosted: _('启动目标', 'Start Goal', '啟動目標'),
  stopHosted: _('停止运行', 'Stop Run', '停止執行'),
  ready: _('就绪', 'Ready', '就緒'),
  ...
  hostedGatewayNotReady: _('Gateway 未就绪，暂不启动', 'Gateway not ready, cannot start', 'Gateway 未就緒，暫不啟動'),
  hostedStarted: _('目标已启动', 'Goal started', '目標已啟動'),
  hostedStopped: _('目标已停止', 'Goal stopped', '目標已停止'),
  hostedAutoStopSignal: _('助手回复包含完成信号，自动停止', 'Assistant reply contains completion signal, auto-stopping', '助手回覆包含完成信號，自動停止'),
  ...
  hostedErrorThreshold: _('连续错误超过阈值', 'Consecutive errors exceeded threshold', '連續錯誤超過閾值'),
  hostedModelNotConfigured: _('目标模式未配置模型（请在 AI 助手页面配置）', 'Goal mode model not configured (please configure in AI Assistant page)', '目標模式未設定模型（請在 AI 助手頁面設定）'),
  hostedApiError: _('API 错误 {code}', 'API Error {code}', 'API 錯誤 {code}'),
  hostedPrefix: _('[目标模式] ', '[Goal Mode] ', '[目標模式] '),
