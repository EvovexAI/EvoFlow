...
export default {
  ...
  undoN: _('↩ 撤销 ({n})', '↩ Undo ({n})'),
  undone: _('已撤销', 'Undone'),
  providerHint: _('服务商是模型的来源（如 OpenAI 兼容接口、DeepSeek 等）。每个服务商下可添加多个模型。标记为「主模型」的将优先使用，其余作为备选自动切换。配置修改后自动保存。', 'Providers are sources of models (e.g. OpenAI-compatible endpoints, DeepSeek). Each provider can have multiple models. The "primary" model is used first; others serve as fallbacks. Changes are auto-saved.', '服務商是模型的來源（如 OpenAI 相容介面、DeepSeek 等）。每個服務商下可新增多個模型。標記為「主模型」的將優先使用，其餘作為備選自動切換。設定修改后自動儲存。'),
  already: _('已有', 'Exists'),
  ...
  baseUrlPlaceholder: _('https://api.openai.com/v1 或兼容端点', 'https://api.openai.com/v1 or compatible endpoint', 'https://api.openai.com/v1 或相容端點'),
  baseUrlHint: _('模型服务的 API 地址，通常以 /v1 结尾；可选官方或任意 OpenAI 兼容端点。Ollama：http://127.0.0.1:11434/v1', 'API endpoint URL, usually ends with /v1; official or any OpenAI-compatible endpoint. Ollama: http://127.0.0.1:11434/v1', '模型服務的 API 位址，通常以 /v1 結尾；可選官方或任意 OpenAI 相容端點。Ollama：http://127.0.0.1:11434/v1'),
  apiKey: _('密钥 (API Key)', 'API Key', '金鑰 (API Key)'),
  apiKeyPlaceholder: _('sk-...', 'sk-...'),
  apiKeyHint: _('访问服务所需的密钥（官方或兼容端均可），留空表示无需认证。与 ChatGPT Plus 订阅无关。', 'Key for the service (official or compatible endpoint). Leave empty if not needed. Unrelated to ChatGPT Plus.', '訪問服務所需的金鑰（官方或相容端均可），留空表示無需認證。與 ChatGPT Plus 訂閱無關。'),
  ...
  visionRecognition: _('开启视觉识别（图片输入）', 'Enable image understanding', '開啟視覺辨識（圖片輸入）'),
  visionRecognitionHint: _(
    '开启后，聊天里可以上传图片一起问。请确认该模型本身支持看图，否则可能无效或效果较差。',
    'When enabled, you can attach images in chat. The model must support vision or results may be poor.',
    '開啟後，聊天裡可以上傳圖片一起問。請確認該模型本身支援看圖，否則可能無效或效果較差。',
  ),
  visionTag: _('视觉', 'Vision', '視覺'),
