      ...
      const run = async () => {
        ...
        do {
          ...
          try {
            ...
            const pendingNewNow = pendingNewSessionRef.current
            ...
            if (action.type === 'clear') {
              ...
              const cur = String(sessionRef.current || '').trim()
              // Belt-and-suspenders: never steal focus while create is in flight,
              // and never clobber a sessionRef that already moved (just-created key).
              if (pendingNewSessionRef.current) {
                ssLog('refresh.ignore-select-pending-new', {
                  refreshId,
                  ignored: skTail(action.sessionKey),
                })
              } else if (cur && cur !== action.sessionKey) {
                ssLog('refresh.ignore-select-sessionref-wins', {
                  refreshId,
                  ignored: skTail(action.sessionKey),
                  keep: skTail(cur),
                })
