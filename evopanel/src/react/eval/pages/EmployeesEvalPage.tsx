...
const QUESTIONS: Question[] = [
  {
    id: 'solo',
    title: '① 单个员工能不能干活',
    ask: '雇进来 → 派任务 → 该停就停 → 真的会用模型做完吗？',
    meaning: '下面每一步跑完会立刻打勾或打叉，你能看见干到哪了。',
    steps: [
      { id: 'sc_employee_lifecycle', label: '雇佣 / 暂停 / 恢复' },
      { id: 'sc_employee_hire_duplicate', label: '重复雇佣会冲突（不能乱覆盖）' },
      { id: 'sc_employee_task_dispatch_ledger', label: '派任务能落到任务本' },
      { id: 'sc_employee_task_pause_rejects_wake', label: '暂停后不能再叫醒' },
      { id: 'sc_employee_task_busy_mutex', label: '忙着时不能叠着再派' },
      { id: 'sc_employee_task_already_done_skip', label: '已结案不会重复派' },
      { id: 'sc_employee_task_live_wake', label: '真实叫醒一次（要 Gateway + 模型）' },
    ],
    passHint: '可以：日常雇佣派发与门禁正常，真叫醒也有结果。',
    failHint: '有步骤失败：看下方打叉的那几步。',
  },
  {
    id: 'collab',
    title: '② 多组织里能不能协同',
    ask: '同一组织能不能互相叫醒？不同组织会不会乱派？',
    meaning: '这一项只验组织边界规则，步骤少、很快。',
    steps: [
      { id: 'sc_employee_task_org_collab_gate', label: '同组织可协作 · 跨组织禁止 · 老板可派' },
    ],
    passHint: '协同规则正确：同组织能协作，跨组织拦得住。',
    failHint: '组织门禁有问题：看下方失败说明。',
  },
]
...
export function EmployeesEvalPage(_props: { onNavigate?: (k: string) => void }) {
  ...
  async function runOneCase(caseId: string, label: string): Promise<StepStatus> {
    ...
    const started = await evalApi.startRun({
      ...
      case_ids: [caseId],
      async_mode: true,
      config: { mode: 'scenario', module: 'employees' },
    })
    ...
    // Live wake can take minutes; others are quick
    const timeout = caseId.includes('live') ? 6 * 60 * 1000 : 3 * 60 * 1000
    ...
    // Gateway adapter normalizes pass → "success" (not "passed")
    ...
    const ok =
      ...
      ((runSt === 'completed' || runSt === 'success') && results.length === 1 && st !== 'failed' && st !== 'error')
    if (ok) {
  ...
  async function runQuestion(q: Question) {
    ...
    if (!steps.length) {
      setSummary(`「${q.title}」没有可跑步骤。请确认 Gateway 已更新到最新评测目录（含组织协同用例）。`)
    ...
    <>
            ...
            >
              <div className="eval-emp-q-head">
                ...
                <div className="eval-emp-verdict">
                  <span className={`eval-badge ${v === 'pass' ? 'ok' : v === 'fail' ? 'fail' : v === 'partial' || v === 'running' ? 'run' : ''}`}>
                    {verdictLabel(v)}
                  </span>
                  <button
                    type="button"
      ...
      <div className="eval-card">
        ...
        <ul style={{ margin: 0, paddingLeft: 18, color: 'var(--eval-muted)', lineHeight: 1.7 }}>
          ...
          <li>
            <strong>部分可以</strong>：有过有挂——常见是前面账本过了，最后「真实叫醒」因模型/网络挂了。
          </li>
          <li>
            <strong>不行</strong>：核心步骤失败，先别当生产可用。
          </li>
