...
`.trim(),
...

  {
    ...
    body: desktopStub(
      [
        ...
        "- `/status` — 看当前模式、会话等摘要",
        "- `/hosted start …`、`/hosted help` — 托管相关；**停止、查状态**在部分版本里可能仍提示「开发中」，若不好用请回到 **EvoPanel** 用 **[托管](/docs/chat/hosted)** 面板操作",
      ...
      [
        ...
        "- `/status`",
        "- `/hosted start …`, `/hosted help` — hosted; **stop/status may still show “in progress” on some builds**—use the EvoPanel **[Hosted](/docs/chat/hosted)** panel when the bot reply is unclear",
        "- `/models`, `/memory`",
        "- `/help`",
        "",
  ...
  },
  {
    ...
    description: {
      zh: "输入栏「托管」：写清目标、设结束方式，一键让助手在同一对话里持续推进。",
      en: "Composer **Hosted**: set a goal and stop rules, then let the assistant keep working in the same thread.",
    },
    body: desktopStub(
      [
        "## 托管适合做什么",
        "当你有一件事希望助手**在同一轮对话里连续做多步**（改代码、查资料、写文档等），又不想每一步都手动点「继续」时，可以用 **托管**。你在小面板里写清**要达成什么**，助手会按对话里的回复持续推进；进展仍出现在主聊天区，托管区用来**开关任务、看状态**。**托管**和输入栏旁的 **记忆** 不是同一个开关：记忆管「聊过的内容要不要接着用」；托管管「要不要让助手自动往下做」。",
        "",
        "## 打开托管面板",
        "1. 先**选中一个会话**（未选中时「托管」按钮可能不可用）。",
        "2. 点输入栏下方的 **「托管」** 按钮，右侧会滑出 **「托管 Agent」** 面板。",
        "3. 再点一次 **「托管」** 按钮或面板上的 **关闭**，可以收起面板；正在跑时也可以先看聊天区，需要时再打开面板点停止。",
        "",
        "## 面板控件说明",
        "### 任务目标",
        "在 **「任务目标」** 多行输入框里，用自然语言写清楚你希望助手**最终做成什么**（越具体越好，例如要改哪些文件、验收标准是什么）。",
        "**正在托管运行时**，这一栏会被锁住不能改；要先点 **「停止托管」**，改好目标后再 **「启动托管」**。",
        "",
        "### 结束条件（界面里的「先到先停」）",
        "这一块用来约定**什么时候算停**。界面上会说明：**「最大轮次」和「定时自动停止」各自独立，满足任意一个就会停**（不是两个加在一起才算）。",
        "- **最大轮次**：用滑条（或旁边的数字框）调大、调小。数字越大，助手允许来回推进的次数越多；调小则更容易较早停在你能接受的范围内。",
        "- **定时自动停止**：打开开关后，会出现一条**时长**滑条，用来设「最多跑多久」；托管跑起来后，界面上会显示**还剩多久**一类的提示。若你**关掉**定时开关，则只靠「最大轮次」或助手判断任务可以结束时的收束。",
        "日常用法：怕跑太久就打开定时；希望尽量做完再把「最大轮次」适当调大，两者搭配着用。",
        "",
        "### 人格风格",
        "在 **温暖 / 专业 / 活泼 / 平静** 里点选一个。托管过程中助手说话的语气会贴近你选的风格；**运行中**不能切换，需先停止再改。",
        "",
        "### 启动与停止",
        "面板底部的大按钮：未运行时显示 **「启动托管」**，跑起来后变为 **「停止托管」**。",
        "最下方还有一行**状态文字**（例如就绪、运行中、出错提示等），看不懂时可以把完整提示复制下来反馈给支持方。",
        "",
        "## 运行期间的配合方式",
        "- 主聊天区会继续出现助手的消息、工具结果等，**像平时聊天一样阅读**即可。",
        "- 若助手在对话里问你问题或请你确认，按聊天里的提示回复；需要紧急刹车时，打开托管面板点 **「停止托管」**。",
        "- 若已在 **设置 → IM 通信** 里接好了飞书等渠道，部分版本会把**完成或异常**用消息推给你（以你当前版本界面为准）。",
        "",
        "## 和快捷指令的关系",
        "习惯打字的用户也可以在输入框里用 `/hosted` 相关指令（见 [快捷指令](/docs/quick-commands)）。**面板操作与聊天指令选一种习惯即可**，不必混用。",
      ],
      [
        "## When Hosted helps",
        "Use **Hosted** when you want the assistant to **keep working across multiple turns in the same chat** (edits, research, writing) without you clicking “continue” every time. You describe the **outcome** in the side panel; progress still appears in the main thread, while the panel is for **starting/stopping and reading status**. **Hosted** is not the same control as **Memory**: memory controls whether the thread keeps context; hosted controls whether the assistant should **keep driving the work forward**.",
        "",
        "## Opening the Hosted panel",
        "1. **Select a thread** first (the **Hosted** button may stay disabled until a session exists).",
        "2. Tap the **Hosted** button under the composer to open the **Hosted agent** drawer.",
        "3. Tap the **Hosted** button again or use **Close** in the header to hide the drawer. While a run is active you can keep reading the chat and reopen the panel when you need **Stop**.",
        "",
        "## Panel controls",
        "### Task goal",
        "Write what you want delivered in the **Task goal** box—the clearer the acceptance criteria, the easier it is for the assistant to finish. The field is **locked while a run is active**; press **Stop hosted**, edit the goal, then **Start hosted** again.",
        "",
        "### Stop conditions (“first signal wins”)",
        "This card explains that **Max rounds** and **Timed auto-stop** are **independent**—**whichever trips first ends the run** (they do not add together).",
        "- **Max rounds**: move the slider (or type in the number box). Higher values allow more back-and-forth; lower values stop sooner.",
        "- **Timed auto-stop**: enable the toggle to reveal a duration slider. While running you should see a **time remaining** style hint. If the toggle is **off**, stopping relies on **max rounds** or the assistant wrapping up naturally.",
        "Practical mix: enable the timer when you are worried about long runs; raise max rounds when you want more room to finish.",
        "",
        "### Persona tone",
        "Pick one of **Warm / Professional / Lively / Calm** for the assistant’s tone during hosted work. You cannot switch mid-run—stop first, change, then start again.",
        "",
        "### Start and stop",
        "The large button reads **Start hosted** when idle and **Stop hosted** while running. A **status line** at the bottom shows ready/running/error text—copy it if you need support.",
        "",
        "## While it is running",
        "- Read the main chat as usual; tool output and assistant replies land there.",
        "- Answer questions the assistant posts in-thread; use **Stop hosted** if you need to abort quickly.",
        "- If **Settings → IM** is configured, some builds push completion or failure notices to Feishu or other channels—follow what your build shows.",
        "",
        "## Slash commands",
        "You can also steer hosted work with `/hosted` commands listed under [Shortcuts](/docs/quick-commands). **Pick either the panel or slash commands**—no need to mix both unless you want to.",
      ],
    ),
  },
