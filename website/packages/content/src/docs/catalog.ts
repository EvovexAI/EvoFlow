...
`.trim(),
  ...
  {
    slug: ["chat", "right-stage"],
    title: { zh: "右侧 Stage", en: "Right stage" },
    description: {
      zh: "聊天右侧统一展示槽位：工作区、思维导图、工作流、实时写入流、资讯大屏等可扩展场景。",
      en: "Unified right-side display slot: workspace, mind map, workflow, live write stream, news dashboard, and more.",
    },
    body: desktopStub(
      [
        "## 是什么",
        "EvoFlow 聊天主区右侧只有一个物理槽位 **Right Stage**。当前展示哪种内容由 **kind** 决定，例如：",
        "- **workspace-browse / workspace-write**：工作区文件树与写入预览",
        "- **mind-map**：思维导图",
        "- **collab-workflow**：协作工作流画布",
        "- **write-stream**：Agent 写文件时的实时内容流（与文件树解耦）",
        "- **news-dashboard**：资讯热榜大屏",
        "",
        "## Agent 如何控制",
        "主助手可使用内置工具 **`stage_set`**：",
        "- `action=show, kind=news-dashboard` 打开资讯面板",
        "- `action=show, kind=write-stream` 打开实时写入流",
        "- `action=stream` 追加写入流文本",
        "- `action=hide` 关闭右侧面板",
        "",
        "## 扩展新的 kind（开发者）",
        "在 EvoPanel 前端调用 **`registerRightStageKind({ kind, render })`** 注册 React 渲染器；布局宽度通过 surface.layout（half / wide / narrow / workspace-write）映射。",
        "注册入口：`evopanel/src/react/right-stage/register-kinds.tsx`。",
      ],
      [
        "## Overview",
        "The chat main area exposes a single right-side **Right Stage** slot. The active **kind** selects the view:",
        "- **workspace-browse / workspace-write** — file tree and write preview",
        "- **mind-map** — knowledge map",
        "- **collab-workflow** — collaboration DAG",
        "- **write-stream** — live agent file write stream",
        "- **news-dashboard** — trending news boards",
        "",
        "## Agent control",
        "The lead agent can call **`stage_set`** with actions `show`, `hide`, `update`, and `stream`.",
        "",
        "## Extending kinds",
        "Call **`registerRightStageKind({ kind, render })`** in EvoPanel to plug in a React renderer. See `evopanel/src/react/right-stage/register-kinds.tsx`.",
      ],
    ),
  },
  {
  ...
  {
    slug: ["employees", "overview"],
    title: { zh: "智能体员工总览", en: "Agent employees overview" },
    description: {
      zh: "智能体员工是持岗待命的智能体，可定时巡检、接收派发、记录工作轨迹。",
      en: "Agent employees are on-duty agents that run scheduled checks, accept dispatch, and log work trails.",
    },
    body: desktopStub(
      [
        "## 智能体员工是什么",
        "**智能体员工**（Agent Employee）与普通**预设角色**的区别在于：预设角色是为聊天对话准备的「人设配置」，而智能体员工是**持岗待命的值班智能体**——它有固定的岗位职责、KPI、工作时长，可以定时巡检，也可以被主智能体派发任务，执行过程和工作结果会被记录在案。",
        "",
        "类比：预设角色像是「同事的简历和能力描述」，智能体员工像是「已经到岗、正在值班的同事」。",
        "",
        "## 岗位 ≠ 智能体",
        "先有**智能体（Agent）**，再有**岗位（Employee）**。一个智能体可以被雇佣到多个岗位吗？不——一个智能体对应一个岗位。但你可以先建智能体，再决定是否把它雇佣成值班岗位。",
        "",
        "流程：",
        "1. 创建智能体（定义人设、可用工具、模型等）",
        "2. 雇佣为岗位（设定职责、KPI、工作区、巡检节奏等）",
        "3. 智能体开始按规则巡检，或接收派发任务",
        "",
        "## 岗位的关键属性",
        "| 属性 | 说明 |",
        "|------|------|",
        "| **agent_code** | 岗位唯一编码（也是对应智能体的编码） |",
        "| **role_name** | 岗位显示名，如「代码巡检员」 |",
        "| **department** | 所属部门，如「工程部」 |",
        "| **responsibilities** | 职责列表，描述该岗位负责什么 |",
        "| **kpis** | 绩效考核指标，可量化 |",
        "| **domain_scope** | 管辖目录路径，限定工作范围 |",
        "| **workspace_path** | 绑定的工作区绝对路径 |",
        "| **autonomy_level** | 自主级别：`approval_for_all`（全部需审批）/ `approval_for_risky`（高风险需审批）/ `full_auto`（全自动） |",
        "| **heartbeat_rrule** | 巡检心跳频率，如每 2 小时一次 |",
        "| **work_schedule_enabled** | 是否启用上下班时间 |",
        "| **work_start_hour / work_end_hour** | 上班 / 下班时间（24 小时制） |",
        "| **max_turns** | 每轮巡检最大步数 |",
        "| **timeout_seconds** | 单轮超时秒数 |",
        "| **daily_budget_usd** | 每日 API 预算上限（0 = 不限） |",
        "| **approval_timeout_minutes** | 待批超时后自动行为（分钟） |",
        "",
        "## 在 EvoFlow 中管理",
        "智能体员工的管理入口在 **系统设置 → 员工管理**（或侧栏 **角色管理** 中的相关页面）。也可以通过 CLI 管理：",
        "",
        "```bash",
        "# 查看所有在岗员工",
        ...
        "",
        "# 雇佣一个智能体为岗位",
        "evoflow employees hire --file hire.json",
        "",
        "# 改岗位信息（职责 / KPI / 心跳）",
        "evoflow employees update code-reviewer --file patch.json",
        "",
        "# 暂停/恢复/归档",
        ...
        "evoflow employees list",
        "```",
        "关注字段：`busy`（是否正忙）、`pending_approvals`（待批事项数）、`gateway_reachable`（Gateway 是否可达）。",
        "",
        "### 某人今天干了啥？",
        "```bash",
        ...
        "```",
        "关注：`rounds[].verdict`（`done` / `incomplete` / `in_progress`）、`rounds[].outcome`（交班结论）、`rounds[].items[]`（事项摘要）。",
        "",
        "### 某一轮具体调了哪些工具？",
        "```bash",
        ...
        "```",
        "关注：`tool_counts`、`steps[]`、`scorecard`。",
        "",
        "## 派发任务给员工",
        "你可以直接通过主会话派发任务给在岗员工，等价于聊天里 @员工：",
        "```bash",
        "evoflow employees dispatch code-agent --goal \"核对 ESLint 是否仍为 0 error\"",
        "```",
        "员工忙碌时会返回 Conflict，先查看进度再决定是否重复派发。",
        "",
        "## 完整示例",
        "以下是一个完整的雇佣 JSON 示例：",
        "```json",
        ...
        '  "responsibilities": [',
        '    "巡检近期提交与 lint/构建健康度",',
        '    "对可自动修复的小问题提出 code_change 事项"',
        '  ],',
        '  "workspace_path": "D:/dev/github/haze-ctrl",',
        '  "domain_scope": ["frontend/", "evopanel/"],',
        '  "kpis": [',
        '    "ESLint 无新增 error",',
        '    {"name": "构建可用", "target": "pass", "probe": "build_ok"}',
        '  ],',
        ...
        '  "role_name": "前端巡检员",',
        '  "reports_to": "tech-lead",',
        '  "responsibilities": ["前端 UI / 性能巡检", "ESLint 与关键页面回归"],',
        '  "kpis": ["ESLint 无新增 error", "首页可交互"],',
        '  "domain_scope": ["evopanel/src/"],',
        '  "heartbeat_rrule": "FREQ=HOURLY;INTERVAL=3",',
        '  "max_turns": 15,',
        '  "work_start_hour": 9,',
        '  "work_end_hour": 21,',
        ...
        "",
        "## 补充说明",
        "岗位配置存储在 `employees/` 目录下（JSON/YAML），与智能体配置分开管理。",
        "更多进阶用法（多员工协作、值班表、考核记录）见 GitHub：[EvovexAI/EvoFlow](https://github.com/EvovexAI/EvoFlow)。",
      ],
      [
        "## What is an agent employee",
        "An **Agent Employee** is an on-duty agent that has a fixed role, KPIs, work schedule, and workspace. Unlike a **preset role** (which is a chat persona), an employee is a **standing duty post**—it runs scheduled checks, accepts dispatch from the lead agent, and logs its work trail.",
        "",
        "Analogy: a preset role is a colleague's resume; an employee is a colleague who is **already on shift**.",
        "",
        "## Agent ≠ Employee",
        "First create an **Agent** (define persona, tools, model), then **Hire** it as an **Employee** (set responsibilities, KPIs, workspace, heartbeat). One agent = one employee post.",
        "",
        "Flow:",
        "1. Create an agent (`evoflow agents create`)",
        "2. Hire as an employee (`evoflow employees hire`)",
        "3. The employee starts running scheduled checks, or accepts dispatch",
        "",
        "## Key employee fields",
        "| Field | Description |",
        "|-------|-------------|",
        "| **agent_code** | Unique employee code (same as the agent code) |",
        "| **role_name** | Display name, e.g. \"Code reviewer\" |",
        "| **department** | Department, e.g. \"Engineering\" |",
        "| **responsibilities** | List of duties |",
        "| **kpis** | Key performance indicators |",
        "| **domain_scope** | Restricted directory paths |",
        "| **workspace_path** | Absolute workspace path |",
        "| **autonomy_level** | `approval_for_all` / `approval_for_risky` / `full_auto` |",
        "| **heartbeat_rrule** | Heartbeat/schedule frequency |",
        "| **work_schedule_enabled** | Enable work hours |",
        "| **work_start_hour / work_end_hour** | Start/end hour (24h) |",
        "| **max_turns** | Max steps per round |",
        "| **timeout_seconds** | Per-round timeout |",
        "| **daily_budget_usd** | Daily API budget cap (0 = unlimited) |",
        "| **approval_timeout_minutes** | Auto action after approval timeout |",
        "",
        "## Management via CLI",
        "```bash",
        "# List all employees",
        ...
        "evoflow employees get code-reviewer",
        "",
        "# Hire an agent as an employee",
        "evoflow employees hire --file hire.json",
        "",
        "# Update employee (responsibilities, KPIs, heartbeat)",
        "evoflow employees update code-reviewer --file patch.json",
        "",
        ...
        "```",
        "",
        "## Observing employee work",
        "### Who's on duty today?",
        "```bash",
        "evoflow employees list",
        "```",
        "Key fields: `busy`, `pending_approvals`, `gateway_reachable`.",
        "",
        "### What did someone do today?",
        "```bash",
        "evoflow employees worklog code-agent",
        "evoflow employees worklog code-agent --day 2026-07-18",
        "```",
        "Key fields: `verdict` (`done` / `incomplete` / `in_progress`), `outcome`, `items[]`.",
        "",
        "### What tools did they call in a round?",
        "```bash",
        "evoflow employees trail code-agent",
        "evoflow employees trail code-agent --round-id round_abc123",
        "```",
        "Key fields: `tool_counts`, `steps[]`, `scorecard`.",
        "",
        "## Dispatching tasks",
        "You can dispatch a task to an employee directly from the main chat:",
        "```bash",
        "evoflow employees dispatch code-agent --goal \"Check ESLint still has 0 errors\"",
        "```",
        "If the employee is busy, a Conflict is returned—check progress first.",
        "",
        "## Sample hire JSON",
        "```json",
        '{',
        ...
        '  "reports_to": "tech-lead",',
        '  "responsibilities": [',
        '    "Review recent commits and lint/build health",',
        '    "Propose code_change items for auto-fixable issues"',
        '  ],',
        '  "workspace_path": "D:/dev/github/haze-ctrl",',
        '  "domain_scope": ["frontend/", "evopanel/"],',
        '  "kpis": [',
        '    "ESLint no new errors",',
        '    {"name": "Build pass", "target": "pass", "probe": "build_ok"}',
        '  ],',
        '  "autonomy_level": "approval_for_all",',
        '  "risk_threshold": "medium",',
        '  "heartbeat_rrule": "FREQ=HOURLY;INTERVAL=2",',
        ...
        "",
        "## Supplementary docs",
        "Employee configs are stored under `employees/` directory (JSON/YAML), separate from agent configs.",
        "Advanced usage (multi-employee collaboration, shift schedules, performance reviews): [EvovexAI/EvoFlow](https://github.com/EvovexAI/EvoFlow).",
      ],
    ),
  },
  {
...
export const docsNavSections: DocNavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { slug: ["chat", "right-stage"], title: { zh: "右侧 Stage", en: "Right stage" } },
  ...
  {
    title: { zh: "智能体员工", en: "Agent employees" },
    items: [
      { slug: ["employees", "overview"], title: { zh: "智能体员工总览", en: "Overview" } },
    ],
  },
  {
