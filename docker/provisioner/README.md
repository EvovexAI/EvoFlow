...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `evo-flow` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `evo-flow` | Kubernetes namespace for sandbox resources |
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `evo-flow` namespace
     - Create/read/delete Services in the `evo-flow` namespace
     - Read Namespaces (to create `evo-flow` if missing)
...
docker compose -p evo-flow-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec evo-flow-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker exec evo-flow-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec evo-flow-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n evo-flow -l sandbox-id=test-001

# Delete sandbox
docker exec evo-flow-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec evo-flow-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec evo-flow-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec evo-flow-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/backend/.evo-flow/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n evo-flow`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n evo-flow`
...
3. **Network Isolation**: Sandbox Pods run in the `evo-flow` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
