...
### 智能体员工（`#/proactive`）
**你能做什么**：让智能体像员工一样定时上岗——巡检工作区、建工作项、等你审批、交班写报告；也可用全局助手 **小V** 左侧选员工直接委派临时目标。
**何时用**：需要长期盯仓库、主动巡检、有审批流程的场景；或临时把事派给某岗并看进度/汇报。
**功能关系**：先在「智能体」页创建角色，再到「智能体员工」页雇佣上岗。小V 一员工一会话；需要细节时再打开工作轨迹。详见 [智能体员工](smart-employees.md)。
...
| IM 通信 | 配置飞书等渠道凭证；可查看员工/小V 飞书专属绑定关系 |
| 记忆 | 查看/编辑记忆数据 |
| 快捷键 | 自定义常用操作快捷键 |
| 外观 | 切换亮色/暗色主题 |
...
│   ├── 应用中心 → 可复用的工作流模板
│   ├── 自动化 → 定时跑固定任务
│   ├── 智能体员工 → 值班岗定时巡检；也可用小V 委派
│   ├── 智能体 → 角色/技能/MCP 管理
│   ├── 上传文档 → RAG 知识库（零散文档）
│   └── 知识库 → Obsidian Vault（本地笔记）
│
├── 全局助手小V → 一员工一会话委派、进度汇报
│
└── 设置弹窗
    ├── 模型配置
    ├── IM 渠道（含员工飞书专属绑定）
