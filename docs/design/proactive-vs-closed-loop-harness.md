...
| **C. 聊天 HARNESS** | 小蜜（`main`）前台传讯 + Plan → 你授权 → `supervisor` 拆派 DAG | `agents/xiaomi/` + collab / supervisor；**值班中间件默认禁用 plan/supervisor** |
...
| 岗位 | agent_code | 角色定位 |
|------|------------|----------|
| **小蜜**（用户全局前台） | `main` | 接待、传讯、分诊；**常驻心跳催办**未结 Task（`agents/xiaomi/duty.py`）；不亲自一线工程 |
