...
左侧菜单「扩展 → Skills」进入技能管理页面，含两个 Tab：
...
| **研究** | `deep-research`、`53ai-news`、`aihot` 等联网调研 |
...
完整清单见 [技能参考](../../reference/skills-reference.md)。
...
**Q：装好的技能在对话里不出现？**
依次检查：① 「Skills」页面该技能是否已启用；② 「Agent 管理」当前角色是否勾选了该技能；③ 触发关键词是否与 `SKILL.md` 描述匹配。

**Q：能不能跨实例同步技能？**
公共技能（`skills/public/`）随仓库分发；自定义技能（`skills/custom/`）需手动同步该目录或重新导入。

**Q：技能调用时报"未授权"？**
对应技能里调用的工具未授权——查看 `SKILL.md` 中提到的工具，回到「Agent 管理」给当前角色补齐工具白名单。
...
- [技能参考](../../reference/skills-reference.md)
