...
本文说明：**EvoFlow Plan 模式**（主控 `plan` 工具）与**项目团队**（`project-*` 内置角色）如何配合，从 0 到 1 完成软件类任务。可与 [Plan 模式使用](./plan-mode.md)、[Agent 管理](./agent-management.md) 对照阅读。
...
| **参考案例** | [媒体 agent 团队协作（Plan 模式）](../assets/媒体agent团队协作%20plan模式完成视频创作.md) | 本文 |
...
- [技能参考](../reference/skills-reference.md)（superpowers-* 技能说明）
...
1. 顶栏选 **「任务规划」**（或开启 Plan / 协作模式）
2. 绑定 **工作区**（本地空目录或新建子目录，例如 `~/projects/todo-lite-ts`）
3. 在 **扩展 → 技能** 中启用相关 `superpowers-*` 技能（简易示例可不强制全开）
...
| 主对话 `recursion_limit` | 2500 | EvoPanel `ws-client`、Channel、Plan 授权执行 |
| 子智能体 `recursion_limit` 上限 | 3000 | `max_turns×8` 推导，封顶 `EVOFLOW_SUBAGENT_RECURSION_LIMIT_MAX` |
| EvoPanel Guardian Plan 保护 | 4h | 执行中 defer 自动 `reload_gateway` |
