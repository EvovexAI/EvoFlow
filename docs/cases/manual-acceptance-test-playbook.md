...
**能力向验收项（创建/管理智能体与技能、自动化、Plan、联网出稿等）** 的编号、命令与 REST 说明见 **[流式验收案例目录（能力向）](stream-acceptance-catalog.md)**，与索引中的故事线文档交叉对照。
...
| **不在此接口内的操作** | 纯表单类：新建角色、新建技能、自动化 TOML、渠道配置等，走 REST/UI；若案例包含这类步骤，文档中区分「非流式步骤」与「流式对话步骤」，**流式步骤**必须按第 4 节记入交互记录。 |
