...
> 文档类型：产品 + 架构说明（解释性文档）  
> 日期：2026-07-14  
> 关联：[`docs/design/app-design.md`](../design/app-design.md)、[`docs/api/apps-api.md`](../api/apps-api.md)、[`docs/explanation/why-evoflow.md`](why-evoflow.md)
...
会话里手动 `plan` / 任务中心看到的 `plan_steps_json`，与 App 跑出来写入的是**同一执行面结构**。App 表只存模板；Task/Subtask 存「这一单」物化结果。详见 `docs/design/app-design.md` §3.1.1 与 `evoflow.collab.app_schema` 模块注释。
...
| 资源 | 路径 |
|------|------|
| 设计稿（数据模型 / 模式细节） | `docs/design/app-design.md` |
| HTTP API | `docs/api/apps-api.md` |
| 平台架构原则 | `docs/explanation/why-evoflow.md` |
