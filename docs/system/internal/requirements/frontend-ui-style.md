...
- 例如常用渐变：
  ...
  - 目标/模态卡片底：`linear-gradient(180deg, rgba(165, 180, 252, 0.26), rgba(255, 255, 255, 0.96))`
...
- 选择控件：模型/智能体/目标/创意/等均应尽量复用同一类体系
...
- 目标弹窗顶部 header 的蓝紫渐变与底部分割线需要保持“很淡”的强度（透明度约 `0.06 ~ 0.08` 区间），不要让顶部显得抢眼。
