...
| **业务与运营人员** | 运营、销售支持、项目经理、行政与综合岗 | 日报周报、巡检、竞品与数据整理；习惯在 IM 里办事 | 自然语言下达任务、定时/目标无人值守、飞书等 IM 内完成对话与结果回推 |
...
独立沙箱后台运行，最长支持 **7×24 小时**（10080 分钟）；实时查看状态与日志；暂停/恢复/终止；结果可追溯；支持**模型通过 `propose_goal` 提议目标方案 → 用户确认后执行**；结束后可向飞书等 IM 推送 Markdown 小结。
...
Plan 确认执行后，Supervisor 将目标展开为**有向无环子任务图（DAG）**；EvoPanel **工作流 / 子任务面板**实时展示各步状态、依赖关系与进度百分比。你可以一眼看到「哪一步在跑、哪一步被阻塞、哪一步已完成」，便于介入、重试或局部重编排——适合研发流水线、运维处置、跨日项目等需要**把复杂链路摊开看清楚**的场景。

### 4.10 模型 Prompt 缓存 · 降本

长任务与多轮工具调用中，系统提示前缀、技能说明块、冻结记忆快照等**刻意保持稳定**，以配合模型侧 **Prompt Caching**（Anthropic、部分 OpenAI 兼容网关等）。在典型多轮会话里，**缓存命中率可达约 90%**；命中的输入 Token 往往按更低费率计费，**可显著节省 API 费用**。EvoPanel **观测 / 调用日志** 中会展示「缓存命中」「写入缓存」等 Token 分项，便于核对实际降本效果。

### 4.11 思维导图 · 排查与复盘

聊天页右侧 **思维导图** 面板由 Agent 在干活过程中**自动维护**——与消息流互补：聊天看「说了什么」，导图看「在想什么、追哪条线」。多轮对话里可快速看清**问题链路**（假设 → 验证 → 分支 → 结论），排查卡点、排除已证伪路径、复盘「从哪一步开始偏了」；Plan 模式与长目标运行中维护最勤。详见 [思维导图](../guides/chat/knowledge-map.md)。

**补充能力（常与上述组合使用）：**

- **飞书 / IM 全链路**：在 IM 侧完成对话、目标、定时、斜杠指令；飞书支持流式卡片更新（其他已接入渠道以文本/卡片为主）。
- **自然语言操作**：目标、自动化可在对话中用自然语言或 `/goal`、`/automation` 创建。
...
- [思维导图](../guides/chat/knowledge-map.md) — 排查链路与多轮复盘
