...
| 9 | **PlanGuardMiddleware** | Filters side-effectful tool calls during planning-like phases (`planning`, `plan_ready`, `awaiting_exec`) |
...
- **Skills path**: `/mnt/skills` → `evo-flow/skills/` directory
...
| **Built-in** | `ask_clarification`, `view_image`, `task` (subagent) |
...
| `POST /api/debug/hang-diagnostics/dump` | Manually write a Gateway hang diagnostic snapshot under `logs/debug/hang-diagnostics/` |
...
- `EVOFLOW_CONFIG_PATH` - Override config.yaml location
- `EVOFLOW_EXTENSIONS_CONFIG_PATH` - Override extensions_config.json location
