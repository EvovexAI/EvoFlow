...
from evoflow.collab.debug_runner import _find_step_by_ref, debug_run_step
from evoflow.collab.expression_resolver import _traverse, resolve_bindings, resolve_expression
from evoflow.collab.workflow_validator import _canonical_cycle_key, _detect_cycles, validate_app_definition
...
class TestBug122GeneratorEmptyGoal:
    ...
    def test_empty_goal_returns_app(self):
        """测试空goal时返回应用"""
        result = generate_app_from_plan(name="Test App", goal="", steps=[])
    ...
    def test_none_goal_returns_app(self):
        """测试None goal时返回应用"""
        result = generate_app_from_plan(name="Test App", goal=None, steps=[])
        assert result is not None
        assert "id" in result
