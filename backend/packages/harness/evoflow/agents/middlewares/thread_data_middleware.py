...
from evoflow.agents.thread_state import ThreadDataState
from evoflow.config.paths import Paths, get_paths
from evoflow.tools.host_direct.workspace_context import bound_workspace_thread_paths
...
class ThreadDataMiddleware(AgentMiddleware[ThreadDataMiddlewareState]):
    ...
    def before_agent(self, state: ThreadDataMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        if existing_thread_data:
            logger.debug("Inherited thread_data present; may realign with bound workspace")
        ...
        paths = bound_workspace_thread_paths(paths, runtime=runtime)

        if existing_thread_data:
            # Re-align inherited sandbox paths when session binds a local workspace root.
            merged = {**existing_thread_data, **paths}
            if merged != existing_thread_data:
                logger.debug("Re-aligned inherited thread_data with bound workspace")
                return {"thread_data": merged}
            return None
