...
from .client import HOST, resolve_active_host, resolve_config_path
...
def cmd_enable(args) -> None:
    ...
    if not block.get("aiPeer"):
        ...
        for key in ("recallMode", "writeFrequency", "sessionStrategy", "contextTokens", "dialecticReasoningLevel", "dialecticDynamic", "dialecticMaxChars", "messageMaxChars", "dialecticMaxInputChars", "saveMessages", "observation"):
            val = default_block.get(key)
...
def cmd_setup(args) -> None:
    ...
    current_deploy = "local" if any(h in (cfg.get("baseUrl") or cfg.get("base_url") or "") for h in ("localhost", "127.0.0.1", "::1")) else "cloud"
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        current = (cfg.get("hosts") or {}).get(_host_key(), {}).get("recallMode") or cfg.get("recallMode") or "hybrid"
        print("\nHoncho recall mode\n" + "─" * 40)
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from evoflow-honcho to Hermes Honcho",
    )
