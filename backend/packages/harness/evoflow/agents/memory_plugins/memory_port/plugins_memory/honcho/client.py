...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        base_url = raw.get("baseUrl") or raw.get("base_url") or os.environ.get("HONCHO_BASE_URL", "").strip() or None
        ...
        session_strategy = host_block.get("sessionStrategy") or raw.get("sessionStrategy", "per-directory")
        host_prefix = host_block.get("sessionPeerPrefix")
        session_peer_prefix = host_prefix if host_prefix is not None else raw.get("sessionPeerPrefix", False)
        ...
        return cls(
            ...
            dialectic_reasoning_level=(host_block.get("dialecticReasoningLevel") or raw.get("dialecticReasoningLevel") or "low"),
            dialectic_dynamic=_resolve_bool(
                host_block.get("dialecticDynamic"),
                raw.get("dialecticDynamic"),
                default=True,
            ),
            dialectic_max_chars=int(host_block.get("dialecticMaxChars") or raw.get("dialecticMaxChars") or 600),
            message_max_chars=int(host_block.get("messageMaxChars") or raw.get("messageMaxChars") or 25000),
            dialectic_max_input_chars=int(host_block.get("dialecticMaxInputChars") or raw.get("dialecticMaxInputChars") or 10000),
            recall_mode=_normalize_recall_mode(host_block.get("recallMode") or raw.get("recallMode") or "hybrid"),
            ...
            observation_mode=_normalize_observation_mode(host_block.get("observationMode") or raw.get("observationMode") or ("unified" if _explicitly_configured else "directional")),
            **_resolve_observation(
                _normalize_observation_mode(host_block.get("observationMode") or raw.get("observationMode") or ("unified" if _explicitly_configured else "directional")),
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError("Honcho API key not found. Get your API key at https://app.honcho.dev, then run 'hermes honcho setup' or set HONCHO_API_KEY. For local instances, set HONCHO_BASE_URL instead.")
    ...
    _is_local = resolved_base_url and ("localhost" in resolved_base_url or "127.0.0.1" in resolved_base_url or "::1" in resolved_base_url)
    if _is_local:
