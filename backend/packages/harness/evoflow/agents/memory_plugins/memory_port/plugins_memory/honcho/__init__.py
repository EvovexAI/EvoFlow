...
CONTEXT_SCHEMA = {
    "name": "honcho_context",
    "description": (
        "Ask Honcho a natural language question and get a synthesized answer. Uses Honcho's LLM (dialectic reasoning) — higher cost than honcho_profile or honcho_search. Can query about any peer: the user (default) or the AI assistant."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
CONCLUDE_SCHEMA = {
    ...
    "description": (
        "Write a conclusion about the user back to Honcho's memory. Conclusions are persistent facts that build the user's profile. Use when the user states a preference, corrects you, or shares something to remember across sessions."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if agent_context in ("cron", "flush") or platform == "cron":
                logger.debug("Honcho skipped: cron/flush context (agent_context=%s, platform=%s)", agent_context, platform)
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        self._session_key = cfg.resolve_session_name(session_title=session_title, session_id=session_id) or session_id or "hermes-default"
    ...
    def system_prompt_block(self) -> str:
        ...
        if not self._manager or not self._session_key:
            ...
            if self._recall_mode == "tools" and self._config:
                return "# Honcho Memory\nActive (tools-only mode). Use honcho_profile, honcho_search, honcho_context, and honcho_conclude tools to access user memory."
            return ""
        ...
        if self._recall_mode == "context":
            header = "# Honcho Memory\nActive (context-injection mode). Relevant user context is automatically injected before each turn. No memory tools are available — context is managed automatically."
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        if self._dialectic_cadence > 1:
            if (self._turn_count - self._last_dialectic_turn) < self._dialectic_cadence:
                logger.debug("Honcho dialectic prefetch skipped: cadence %d, turns since last: %d", self._dialectic_cadence, self._turn_count - self._last_dialectic_turn)
                return
    ...
    def handle_tool_call(self, tool_name: str, args: dict, **kwargs) -> str:
        ...
        try:
            if tool_name == "honcho_profile":
                ...
                result = self._manager.search_context(self._session_key, query, max_tokens=max_tokens)
                if not result:
