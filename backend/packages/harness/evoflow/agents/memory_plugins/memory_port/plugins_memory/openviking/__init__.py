...
class _VikingClient:
    ...
    def get(self, path: str, **kwargs) -> dict:
        resp = self._httpx.get(self._url(path), headers=self._headers(), timeout=_TIMEOUT, **kwargs)
        resp.raise_for_status()
        return resp.json()

    def post(self, path: str, payload: dict = None, **kwargs) -> dict:
        resp = self._httpx.post(self._url(path), json=payload or {}, headers=self._headers(), timeout=_TIMEOUT, **kwargs)
        resp.raise_for_status()
        return resp.json()
...
SEARCH_SCHEMA = {
    "name": "viking_search",
    "description": (
        "Semantic search over the OpenViking knowledge base. Returns ranked results with viking:// URIs for deeper reading. Use mode='deep' for complex queries that need reasoning across multiple sources, 'fast' for simple lookups."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
REMEMBER_SCHEMA = {
    ...
    "description": ("Explicitly store a fact or memory in the OpenViking knowledge base. Use for important information the agent should remember long-term. The system automatically categorizes and indexes the memory."),
    "parameters": {
        "type": "object",
        "properties": {
...
ADD_RESOURCE_SCHEMA = {
    ...
    "description": ("Add a URL or document to the OpenViking knowledge base. Supports web pages, GitHub repos, PDFs, markdown, code files. The system automatically parses, indexes, and generates summaries."),
    "parameters": {
        "type": "object",
        "properties": {
