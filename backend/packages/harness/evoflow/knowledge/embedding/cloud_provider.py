...
def _resolve_connection(model_config: ModelConfig) -> tuple[str, str, str, float]:
    ...
    model_name = (getattr(model_config, "model", "") or "").strip() or DEFAULT_EMBEDDING_MODEL
    timeout = float(getattr(model_config, "request_timeout", None) or DEFAULT_TIMEOUT)
    ...
    if not base_url or not api_key:
        try:
            ...
            if primary_name:
                ...
                if primary:
                    if not base_url:
                        base_url = (getattr(primary, "base_url", "") or "").strip().rstrip("/")
                    if not api_key:
                        api_key = resolve_and_sanitize_api_key(getattr(primary, "api_key", "") or "") or ""
                    if timeout == DEFAULT_TIMEOUT:
                        timeout = float(getattr(primary, "request_timeout", None) or DEFAULT_TIMEOUT)
        except Exception:
...
class CloudEmbeddingProvider(EmbeddingProvider):
    ...
    async def embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        base_url, api_key, model_name, timeout = _resolve_connection(self.model_config)

        if not base_url:
            raise EmbeddingError("No base_url configured for embedding model. Set base_url in Settings → Models for the embedding model, or ensure a primary chat model with base_url is configured.")
        if not api_key:
            raise EmbeddingError("No api_key configured for embedding model. Set api_key in Settings → Models for the embedding model, or ensure a primary chat model with api_key is configured.")
...
def _parse_embeddings_response(body: dict[str, Any], *, n_inputs: int) -> list[list[float]]:
    ...
    if isinstance(data, list) and data:
        ...
        embeddings = [_normalize_embedding_vector(item["embedding"]) for item in data_sorted if isinstance(item, dict)]
        ...
        raise EmbeddingError(f"Embedding API returned {len(embeddings)} embeddings for {n_inputs} inputs")
    ...
    raise EmbeddingError(f"Embedding API response missing 'data' embeddings: {str(body)[:200]}")
    ...
    *,
    ...
    if multimodal:
        ...
        for text in inputs:
            ...
            out.append((await _post_embeddings(url, headers, payload, timeout, n_inputs=1))[0])
        return out
    ...
    *,
    ...
    for attempt in range(4):
        try:
            ...
            last_timeout = exc
            if attempt >= 3:
                raise EmbeddingError(f"Embedding API request timed out after {timeout}s: {exc}") from exc
            await asyncio.sleep(min(30.0, 2.0**attempt))
            continue
        except httpx.ConnectError as exc:
            raise EmbeddingError(f"Cannot connect to embedding API at {url}: {exc}") from exc
        ...
        if resp.status_code == 200:
            try:
                ...
                raise EmbeddingError(f"Failed to parse embedding API response: {exc}") from exc
        ...
        if resp.status_code == 429:
            raise EmbeddingError(f"Embedding API rate limit (429): {error_detail}. Consider reducing batch size or adding retries.") from None
        if resp.status_code == 401:
            raise EmbeddingError(f"Embedding API auth error (401): check api_key. {error_detail}") from None
        raise EmbeddingError(f"Embedding API returned HTTP {resp.status_code}: {error_detail}") from None

    if last_timeout is not None:
        raise EmbeddingError(f"Embedding API request timed out after {timeout}s: {last_timeout}") from last_timeout
