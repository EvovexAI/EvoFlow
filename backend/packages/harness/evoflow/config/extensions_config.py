...
class ExtensionsConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path | None:
        ...
        2. If provided `EVOFLOW_EXTENSIONS_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("EVOFLOW_EXTENSIONS_CONFIG_PATH"):
            path = Path(os.getenv("EVOFLOW_EXTENSIONS_CONFIG_PATH"))
            if not path.exists():
                raise FileNotFoundError(f"Extensions config file specified by environment variable `EVOFLOW_EXTENSIONS_CONFIG_PATH` not found at {path}")
            return path
        else:
    ...
    @classmethod
    def from_db(cls) -> "ExtensionsConfig":
        """Load MCP + skill enablement from SQLite (seeded from extensions JSON on first app load)."""
        from evoflow.persistence.bootstrap import extensions_dict_from_db
        from evoflow.persistence.db import get_db

        get_db()
        payload = extensions_dict_from_db()
        cls.resolve_env_variables(payload)
        return cls.model_validate(payload)

    @classmethod
    def from_file(cls, config_path: str | None = None) -> "ExtensionsConfig":
        """Load extensions config from SQLite.

        Legacy name kept for callers; ``config_path`` is ignored. Prefer :meth:`from_db`.
        """
        del config_path
        return cls.from_db()
    ...
    def resolve_env_variables(cls, config: dict[str, Any]) -> dict[str, Any]:
        ...
        Supports ``$VAR`` and ``${VAR}`` (same as common shell / docker-compose style).
        Unresolved placeholders become empty strings so MCP env does not receive literal ``$VAR``.
        """
        for key, value in config.items():
            if isinstance(value, str):
                config[key] = cls._resolve_env_string(value)
    ...
    def _resolve_env_string(value: str) -> str:
        ...
        if text.startswith("$") and not text.startswith("${"):
            env_value = os.getenv(text[1:])
            if env_value is None:
                return ""
            return ExtensionsConfig._resolve_env_string(env_value)
...
def save_extensions_to_db(config: ExtensionsConfig) -> None:
    """Persist MCP servers and skill enablement to SQLite (authoritative store)."""
    from evoflow.persistence import config_repositories as cfg_repo

    mcp = {name: server.model_dump() for name, server in config.mcp_servers.items()}
    cfg_repo.replace_mcp_servers(mcp)
    skills = {name: {"enabled": skill_config.enabled} for name, skill_config in config.skills.items()}
