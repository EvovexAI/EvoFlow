...
_apps_columns_cache: set[str] | None = None


def _evoflow_apps_columns() -> set[str]:
...
def reset_apps_columns_cache_for_tests() -> None:
    global _apps_columns_cache
    _apps_columns_cache = None
...
_LIST_SUMMARY_SKIP_COLS = frozenset(
    {
        "canvas_json",
        "flowchart_mermaid",
...
def _light_steps_for_list(steps_raw: Any) -> list[dict[str, Any]]:
    """Keep only fields needed for list UI / agent·skill usage maps (no descriptions)."""
    ...
    for raw in steps_raw:
        if not isinstance(raw, dict):
            continue
        light = {k: raw[k] for k in _STEP_SUMMARY_KEYS if k in raw and raw[k] is not None}
        if light:
            out.append(light)
    return out
...
def list_apps(
    ...
    *,
    ...
    ``summary=True`` (default): omit canvas / heavy text fields so list endpoints
    stay small. Use ``summary=False`` only when callers need full definitions.
    """
    conn = get_db()
    params: list[Any] = []
    where = []
    ...
    table_cols = _evoflow_apps_columns()
    if summary:
        select_cols = [c for c in sorted(table_cols) if c not in _LIST_SUMMARY_SKIP_COLS]
        # Prefer stable column order for readability in EXPLAIN / logs
        preferred = [
            "id",
            "name",
            "description",
        ...
        ordered = [c for c in preferred if c in select_cols]
        ordered.extend(c for c in select_cols if c not in ordered)
    ...
    for row in cur.fetchall():
        doc = dict(zip(cols, row, strict=False))
        parameters = _json_load(doc.pop("parameters_json", "[]"))
        if not isinstance(parameters, list):
            parameters = []
        steps_raw = _json_load(doc.pop("steps_json", "[]"))
        tags = _json_load(doc.pop("tags_json", "[]"))
        if not isinstance(tags, list):
            tags = []
        doc["auto_run"] = bool(doc.get("auto_run"))
        if "owner_scope_id" in doc:
            doc["owner_scope_id"] = str(doc.get("owner_scope_id") or "").strip() or None
        if "org_id" in doc:
            doc["org_id"] = str(doc.get("org_id") or "").strip() or None

        if summary:
            steps = _light_steps_for_list(steps_raw)
            doc["parameters"] = parameters
            doc["parameter_count"] = len(parameters)
            doc["steps"] = steps
            doc["step_count"] = len(steps)
            doc["canvas"] = {}
            doc["tags"] = tags
        ...
        doc["parameters"] = parameters
        doc["steps"] = steps_raw if isinstance(steps_raw, list) else []
        doc["validation_template"] = _json_load(doc.pop("validation_template_json", "[]"))
        doc["tags"] = tags
        canvas_raw = doc.pop("canvas_json", "{}")
        canvas = _json_load(canvas_raw)
