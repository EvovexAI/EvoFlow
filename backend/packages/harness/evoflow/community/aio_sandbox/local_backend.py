...
from evoflow.utils.network import get_free_port, release_port
...
class LocalContainerBackend(SandboxBackend):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            container_prefix: Prefix for container names (e.g., "evo-flow-sandbox").
    ...
    def create(self, thread_id: str, sandbox_id: str, extra_mounts: list[tuple[str, str, bool]] | None = None) -> SandboxInfo:
        ...
        sandbox_host = os.environ.get("EVOFLOW_SANDBOX_HOST", "localhost")
        return SandboxInfo(
    ...
    def _start_container(
        ...
        # Docker-specific security hardening.
        # - --read-only: root filesystem is read-only; only tmpfs/explicit
        #   mounts are writable (defense in depth against arbitrary writes).
        # - --cap-drop ALL: drop all Linux capabilities (no privileged syscalls).
        # - --security-opt no-new-privileges: forbid privilege escalation.
        # (seccomp=unconfined was previously set to avoid blocking syscalls used
        #  by the sandbox runtime; the default seccomp profile is now relied on
        #  together with cap-drop ALL for a tighter boundary.)
        ...
        # Writable tmpfs for /mnt/user-data so that the read-only root
        # filesystem still allows user-data writes.  The per-thread workspace /
        # uploads / outputs bind-mounts (added below as -v) overlay specific
        # subdirectories under /mnt/user-data and take precedence for those
        # paths; this tmpfs covers any other /mnt/user-data writes the sandbox
        # runtime may perform at startup before the bind-mounts are populated.
        # NOTE: bind-mounts under /mnt/user-data/* are applied after this and
        # override the tmpfs for their specific subdirectories.
        if self._runtime == "docker":
            cmd.extend(["--tmpfs", "/mnt/user-data:rw,size=512m"])
