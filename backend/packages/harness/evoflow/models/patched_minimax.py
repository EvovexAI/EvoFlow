...
``langchain_openai.ChatOpenAI`` currently ignores that field, so EvoFlow's
...
which EvoFlow already understands.
"""
...
class PatchedChatMiniMax(VendorRoundtripChatMixin, ChatOpenAI):
    ...
    def _create_chat_result(
        ...
        if isinstance(response, dict):
            ...
            # Non-JSON / plain-text bodies must not call ``.model_dump()``
            # (exact failure: ``'str' object has no attribute 'model_dump'``).
            response_dict = {}
        elif hasattr(response, "model_dump") and callable(getattr(response, "model_dump", None)):
            try:
                dumped = response.model_dump()
            except Exception:
                dumped = None
            response_dict = dumped if isinstance(dumped, dict) else {}
        else:
            response_dict = {}
