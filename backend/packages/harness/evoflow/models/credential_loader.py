"""Auto-load credentials from host CLI auth files (optional).

Implements two credential strategies:
  1. Anthropic CLI OAuth token from explicit env vars or an exported credentials file
  ...
  2. ChatGPT desktop/CLI token from ``~/.codex/auth.json`` (path owned by that CLI)
...
def load_codex_cli_credential() -> runtimeCliCredential | None:
    """Load ChatGPT CLI auth from ``~/.codex/auth.json`` (or ``$CODEX_AUTH_PATH``)."""
    cred_path = _resolve_credential_path("CODEX_AUTH_PATH", ".codex/auth.json")
    data = _load_json_file(cred_path, "runtime CLI credentials")
    if data is None:
        return None
    ...
    if not access_token:
        logger.debug("runtime CLI credentials file exists but no token found")
        return None

    logger.info("Loaded runtime CLI credential")
    return runtimeCliCredential(
        access_token=access_token,
