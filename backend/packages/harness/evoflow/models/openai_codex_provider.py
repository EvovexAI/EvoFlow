"""Custom upstream agent runtime provider using ChatGPT runtime Responses API.

Uses runtime CLI OAuth tokens with chatgpt.com/backend-api/codex/responses endpoint.
This is the same endpoint that the runtime CLI uses internally.
...
class runtimeChatModel(BaseChatModel):
    """LangChain chat model using ChatGPT runtime Responses API.
    ...
    def model_post_init(self, __context: Any) -> None:
        """Auto-load runtime CLI credentials."""
        ...
        if cred:
            ...
            logger.info(f"Using runtime CLI credential (account: {self._account_id[:8]}...)")
        else:
            raise ValueError("runtime CLI credential not found. Expected ~/.codex/auth.json or CODEX_AUTH_PATH.")

        super().model_post_init(__context)

    def _load_codex_auth(self) -> runtimeCliCredential | None:
        """Load access_token and account_id from runtime CLI auth."""
        return load_codex_cli_credential()

    @classmethod
    def _normalize_content(cls, content: Any) -> str:
        """Flatten LangChain content blocks into plain text for runtime."""
    ...
    def _call_codex_api(self, messages: list[BaseMessage], tools: list[dict] | None = None) -> dict:
        """Call the runtime Responses API and return the completed response."""
        ...
        for attempt in range(1, self.retry_max_attempts + 1):
            try:
                ...
                if e.response.status_code in (429, 500, 529):
                    ...
                    logger.warning(f"runtime API error {e.response.status_code}, retrying {attempt}/{self.retry_max_attempts} after {wait_ms}ms")
    ...
    def _stream_response(self, headers: dict, payload: dict) -> dict:
        """Stream SSE from runtime API and collect the final response."""
        ...
        if not completed_response:
            raise RuntimeError("runtime API stream ended without response.completed event")
    ...
    def _parse_sse_data_line(line: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.debug(f"Skipping non-JSON runtime SSE frame: {raw_data}")
            return None
    ...
    def _parse_response(self, response: dict) -> ChatResult:
        """Parse runtime Responses API response into LangChain ChatResult."""
        content = ""
        tool_calls = []
        invalid_tool_calls = []
        reasoning_content = ""
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        """Generate a response using runtime Responses API."""
        from evoflow.models.vendor_roundtrip import complete_vendor_roundtrip
        ...
        try:
            ...
            complete_vendor_roundtrip(
                ...
                error=None,
                raw_extra={"codex_response": response},
            )
            return result
