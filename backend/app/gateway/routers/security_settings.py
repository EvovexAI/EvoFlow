...
async def get_security_center_settings(request: Request) -> SecuritySettingsResponse:
    ...
    # Merge detected runtime versions into the response
    detected = _detect_runtime_versions()
    if detected:
        rt = dict(settings.get("builtin_runtime", {}))
        for key, info in detected.items():
            existing = rt.get(key, {})
            rt[key] = {
                "enabled": existing.get("enabled", True),
                "path": info.get("path", existing.get("path", "")),
                "version": info.get("version", existing.get("version", "")),
            }
        settings = {**settings, "builtin_runtime": rt}
