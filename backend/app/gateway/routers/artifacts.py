...
def _extract_file_from_skill_archive(zip_path: Path, internal_path: str) -> bytes | None:
    """Extract a file from a .zip skill archive.

    Args:
        zip_path: Path to the .zip file (skill archive).
...
async def get_artifact(thread_id: str, path: str, request: Request, download: bool = False) -> Response:
    ...
    # Check if this is a request for a file inside a skill archive (e.g., xxx.skill/SKILL.md or xxx.zip/SKILL.md)
    if ".skill/" in path or ".zip/" in path:
        # Determine the marker and split the path
        if ".skill/" in path:
            skill_marker = ".skill/"
        else:
            skill_marker = ".zip/"
        marker_pos = path.find(skill_marker)
        skill_file_path = path[: marker_pos + len(skill_marker.rstrip("/"))]  # e.g., "mnt/user-data/outputs/my-skill.skill"
