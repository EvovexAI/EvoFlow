    ...
    for tid, data in _load_automation_tomls():
        if filter_ctx is not None:
            try:
                ...
                continue
        # Use last_run / last_status on the automation row — avoid N× list_automation_runs.
        row = enrich_automation_for_api({"id": tid, **data}, latest_run=None)
        out.append(row)
    return out
...
@router.get("/runs")
async def automation_runs_recent(
    request: Request,
    limit: int = Query(200, ge=1, le=500),
) -> dict[str, Any]:
    """Newest automation runs across tasks (one SQL) for the runs tab."""
    from evoflow.persistence import automation_repositories as auto_repo

    # Light visibility filter — skip enrich_automation_for_api (preview_schedule per row).
    ...
    for tid, _data in auto_repo.list_automations():
        if filter_ctx is None:
            visible_ids.add(tid)
            continue
        try:
            if auto_repo.automation_visible_to_principal(
                tid,
                is_admin=filter_ctx["is_admin"],
                personal_scope=filter_ctx["personal_scope"],
                org_scope=filter_ctx["org_scope"],
    ...
    runs = auto_repo.list_recent_automation_runs(limit=limit)
    filtered = [r for r in runs if str(r.get("task_id") or "") in visible_ids]
    return {"runs": filtered, "total": len(filtered)}
