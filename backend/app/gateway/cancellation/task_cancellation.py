...
def mark_subtasks_cancelled(project_id: str, task_id: str, cancelled_by: str = "user") -> list[str]:
    ...
    Sets both the in-memory cancellation flag AND persists ``status=cancelled``
    onto each subtask row in project storage so the UI reflects the change
    ...
    """
    from evoflow.collab.storage import find_main_task, get_project_storage, main_task_mutation_lock
    from evoflow.persistence.task_repositories import save_task_bundle

    storage = get_project_storage()
    mid = str(task_id or "").strip()
    if not mid:
        return []

    # Serialize with other main-task mutations to avoid lost writes
    with main_task_mutation_lock(mid):
        ...
        for i, subtask in enumerate(subtasks):
            ...
            if status in ("completed", "failed", "cancelled", "done", "canceled"):
                continue

            # Memory flag (cooperative cancellation signal for workers)
            if mark_task_cancelled(subtask_id, cancelled_by, "parent_task_cancelled"):
                subtask_ids.append(subtask_id)

            # Persist status onto subtask row so UI shows it immediately
            subtasks[i] = {
                **subtask,
                "status": "cancelled",
                "cancelled_at": subtask.get("cancelled_at") or now,
                "cancelled_by": cancelled_by,
                "cancellation_reason": "parent_task_cancelled",
            }
            changed = True
        ...
        if changed:
            ...
            try:
                save_task_bundle(mid, project)
            except Exception:
                logger.exception("mark_subtasks_cancelled: failed to persist task_id=%s", mid)
