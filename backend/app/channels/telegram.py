...
class TelegramChannel(Channel):
    ...
    def __init__(self, bus: MessageBus, config: dict[str, Any]) -> None:
        ...
        self._last_bot_message_lock = threading.Lock()
        self._poll_shutdown_done = False
    ...
    async def stop(self) -> None:
        ...
        if self._tg_loop and self._tg_loop.is_running() and not self._tg_loop.is_closed():
            ...
            try:
                ...
                logger.exception("[Telegram] shutdown timed out or failed")
        if self._thread:
    ...
    async def send(self, msg: OutboundMessage, *, _max_retries: int = 3) -> None:
        ...
        with self._last_bot_message_lock:
    ...
    async def _shutdown_polling(self) -> None:
        if self._poll_shutdown_done or not self._application:
            return
        try:
    ...
    def _run_polling(self) -> None:
        ...
        try:
            ...
            if not self._poll_shutdown_done:
                try:
                    self._tg_loop.run_until_complete(self._shutdown_polling())
                except Exception:
                    logger.exception("Error during Telegram shutdown fallback")
    ...
    async def _cmd_start(self, update, context) -> None:
        ...
        if not self._check_user(update.effective_user.id):
            return
        await update.message.reply_text("Welcome to EvoFlow! Send me a message to start a conversation.\nType /help for available commands.")

    async def _process_incoming_with_reply(self, chat_id: str, msg_id: int, inbound: InboundMessage) -> None:
        await self.bus.publish_inbound(inbound)
        asyncio.create_task(self._send_running_reply(chat_id, msg_id))
    ...
    async def _cmd_generic(self, update, context) -> None:
        ...
        if update.effective_chat.type == "private":
            ...
            topic_id = str(reply_to.message_id) if reply_to else None
    ...
    async def _on_text(self, update, context) -> None:
        ...
        # topic_id determines which EvoFlow thread the message maps to.
        # In private chats, use None so that all messages share a single
        # thread (the store key becomes "channel:chat_id").
        # In group chats, replies anchor a thread; otherwise share one group thread.
        if update.effective_chat.type == "private":
            topic_id = None
        else:
            reply_to = update.message.reply_to_message
