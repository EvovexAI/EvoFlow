...
class SlackChannel(Channel):
    ...
    async def send(self, msg: OutboundMessage, *, _max_retries: int = 3) -> None:
        ...
        for attempt in range(_max_retries):
            try:
                ...
                if msg.thread_ts:
                    await self._add_reaction(msg.chat_id, msg.thread_ts, "white_check_mark")
                return
        ...
        if msg.thread_ts:
            try:
                await self._add_reaction(msg.chat_id, msg.thread_ts, "x")
            except Exception:
                pass
    ...
    async def _add_reaction(self, channel_id: str, timestamp: str, emoji: str) -> None:
    ...
    async def _send_running_reply(self, channel_id: str, thread_ts: str) -> None:
        """Send a 'Working on it...' reply in the thread."""
        if not self._web_client:
            return
        try:
            await asyncio.to_thread(
    ...
    async def _handle_incoming_async(
        self,
        channel_id: str,
        thread_ts: str,
        inbound,
        ...
        await self._add_reaction(channel_id, event_ts, "eyes")
        await self._send_running_reply(channel_id, thread_ts)
    ...
    def _handle_message_event(self, event: dict) -> None:
        ...
        thread_ts = raw_thread_ts or event.get("ts", "")
        ...
        inbound.topic_id = raw_thread_ts

        if self._loop and self._loop.is_running() and not self._loop.is_closed():
            asyncio.run_coroutine_threadsafe(
                self._handle_incoming_async(channel_id, thread_ts, inbound, event.get("ts", thread_ts)),
                self._loop,
            )
