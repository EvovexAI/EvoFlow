# Copilot Onboarding Instructions for EvoFlow
...
EvoFlow is a full-stack "super agent harness".
...
- `nginx` (optional; used only for the unified local entry on `:2026`)
...
- `backend/packages/harness/evoflow/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/evoflow/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/evoflow/subagents/` - subagent registry/execution
- `backend/packages/harness/evoflow/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`evoflow.agents:make_lead_agent`)
