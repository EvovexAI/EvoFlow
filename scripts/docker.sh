...
COMPOSE_CMD="docker compose -p evo-flow-dev -f docker-compose-dev.yaml"
...
detect_sandbox_mode() {
    ...
    if [[ "$sandbox_use" == *"NoopSandboxProvider"* ]] || [[ "$sandbox_use" == *"LocalSandboxProvider"* ]]; then
        echo "local"
    elif [[ "$sandbox_use" == *"evoflow.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
init() {
    echo "=========================================="
    echo "  EvoFlow Init — Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting EvoFlow Docker Development"
    echo "=========================================="
    echo ""
    ...
    # Set EVOFLOW_ROOT for provisioner if not already set
    if [ -z "$EVOFLOW_ROOT" ]; then
        export EVOFLOW_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting EVOFLOW_ROOT=$EVOFLOW_ROOT${NC}"
        echo ""
    fi
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting EvoFlow.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
    ...
    echo ""
    echo "=========================================="
    echo "  EvoFlow Docker is starting!"
    echo "=========================================="
    echo ""
...
stop() {
    # EVOFLOW_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" evo-flow-sandbox 2>/dev/null || true
...
restart() {
    echo "========================================"
    echo "  Restarting EvoFlow Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "EvoFlow Docker Management Script"
    echo ""
