...
help:
	...
	@echo "  make openapi-export  - Regenerate docs/generated/openapi-gateway.json"
	@echo ""
	@echo "CI (local, same as GitHub Actions Unit Tests + Lint + Docs):"
	@echo "  make ci-local          - Run scripts/ci-local.sh (on Windows uses Git Bash)"
	@echo "  make ci-local CI_LOCAL_ARGS=--no-docs   - skip MkDocs + OpenAPI diff"
	@echo "  make setup-git-hooks   - One-time: core.hooksPath=scripts/git-hooks (pre-commit: ci-local --quick)"
...
clean: stop
	@echo "Cleaning up..."
	@-rm -rf backend/.evo-flow 2>/dev/null || true
...
# Mirrors .github/workflows backend-unit-tests.yml + lint-check.yml + docs.yml (not sync-public.yml).
# Optional args for bash only: make ci-local CI_LOCAL_ARGS=--no-docs
ci-local:
ifeq ($(OS),Windows_NT)
	@call scripts\run-with-git-bash.cmd ./scripts/ci-local.sh $(CI_LOCAL_ARGS)
else
	@./scripts/ci-local.sh $(CI_LOCAL_ARGS)
endif

# One-time per clone: versioned hooks (scripts/git-hooks/) run on commit; no copy to .git/hooks.
setup-git-hooks:
ifeq ($(OS),Windows_NT)
	@call scripts\run-with-git-bash.cmd ./scripts/setup-git-hooks.sh
else
