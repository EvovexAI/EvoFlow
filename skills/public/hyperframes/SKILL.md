...
| `/general-video`           | **Anything else** — longer or multi-scene pieces, a static loop / poster, a custom composition, or a general site tour when no specialized workflow fits               |
...
- **A URL or script** — markets a specific product (even just naming the site) → `/product-launch-video`; a general non-product site → `/general-video`; a GitHub PR link → `/pr-to-video`; explains a concept with no product / site → `/faceless-explainer`. Genuinely unclear product-vs-topic, or launch-vs-general-site → ask one question.
