...
# GSAP is NOT vendored (GreenSock license). Fetch into the work dir when composing:
```
...
3. **No native tool** (runtime CLI, plain text-only runtimes) — **ask
...
**Channel B — plain-text fallback** (runtime CLI, runtimes without a
...
# GSAP not shipped in-repo (GreenSock Standard License). Example fetch for local render:
mkdir -p "$WORK_DIR/public/vendor"
curl -fsSL -o "$WORK_DIR/public/vendor/gsap.min.js" "https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/gsap.min.js"
# Review https://gsap.com/standard-license — using GSAP is your responsibility.
