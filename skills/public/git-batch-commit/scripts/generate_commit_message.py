        ...
        if filepath.endswith("SKILL.md"):
            try:
                ...
                result = subprocess.run(
                    ...
                    capture_output=True,
                    text=True,
                    cwd="/Users/example/Library/Application Support/example-app/skills/legal-skills",
                )
...
def analyze_markdown_changes(
    added: List[str], removed: List[str], filename: str
) -> str:
    """分析 Markdown 文件变更。"""
...
def infer_intent_from_function_name(func_name: str) -> str | None:
    ...
    for prefix, action in FUNCTION_PREFIX_ACTIONS.items():
        if func_name.startswith(prefix):
            ...
            if (
                rest.lower().endswith(simple_suffixes)
                or rest.lower() in simple_suffixes
            ):
                return action
...
def detect_modified_functions(
    added_funcs: List[str], removed_funcs: List[str]
) -> List[str]:
...
def main():
    """命令行使用的主入口。"""
    parser = argparse.ArgumentParser(description="生成约定式提交信息")
    parser.add_argument("--category", type=str, help="变更类别 (deps, docs, feat 等)")
    ...
    if args.category and args.files:
        ...
        print(
            "用法: generate_commit_message.py --category <类型> --files <文件1> [文件2...]"
        )
