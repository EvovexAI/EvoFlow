...
def stage_files(files: List[str]) -> bool:
    ...
    if not files:
        return True
    try:
        subprocess.run(["git", "add"] + files, capture_output=True, check=True)
        return True
...
def unstage_files(files: List[str]) -> bool:
    ...
    try:
        subprocess.run(
            ["git", "reset", "HEAD"] + files, capture_output=True, check=True
        )
        return True
    except subprocess.CalledProcessError as e:
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="显示将要提交的内容而不实际提交"
    )
    parser.add_argument(
        "--yes",
        "-y",
