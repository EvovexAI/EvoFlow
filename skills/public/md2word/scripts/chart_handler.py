...
def preprocess_mermaid_code(mermaid_code: str) -> str:
    ...
    s = re.sub(
        r"(?m)(?P<brace>[\[\({\>])(?P<quote>\"?\s*)(?P<num>\d+)\.\s",
        _repl_number_dot,
        s,
    )
...
def try_local_mermaid_render(
    insert_image_func, get_image_path_func, mermaid_code, md_file_path
):
    ...
    try:
        ...
        mmdc_path = (
            mmdc_env
            if mmdc_env
            else os.path.join(script_dir, "node_modules", ".bin", "mmdc")
        )
        ...
        print(
            f"✅ 本地Mermaid图表渲染成功！图片已保存至: {os.path.relpath(output_png_path)}"
        )
        return True
...
def create_mermaid_chart(
    ...
    # 首先尝试本地渲染
    local_success = try_local_mermaid_render(
        insert_image_func, get_image_path_func, mermaid_code, md_file_path
    )
    if local_success:
        return
