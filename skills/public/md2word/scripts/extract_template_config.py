...
def extract_template_config(template_path: Path) -> Dict[str, Any]:
    ...
    if normal is not None:
        ...
        config["fonts"]["default"]["size"] = (
            _pt(normal_font.size) or config["fonts"]["default"]["size"]
        )
        config["fonts"]["default"]["color"] = (
            _to_hex(normal_font.color.rgb) or config["fonts"]["default"]["color"]
        )
        ...
        config["paragraph"]["first_line_indent"] = (
            _pt(normal_para.first_line_indent)
            or config["paragraph"]["first_line_indent"]
        )
        config["paragraph"]["align"] = (
            _alignment_to_str(normal_para.alignment) or config["paragraph"]["align"]
        )
        config["table"]["body"]["font"] = font_name
        config["table"]["body"]["size"] = config["fonts"]["default"]["size"]

    for level, style_name in enumerate(
        ["Heading 1", "Heading 2", "Heading 3", "Heading 4"], start=1
    ):
    ...
    if code_style is not None:
        ...
        config["code_block"]["content"]["font"] = (
            code_font.name or config["code_block"]["content"]["font"]
        )
        config["code_block"]["content"]["size"] = (
            _pt(code_font.size) or config["code_block"]["content"]["size"]
        )
        config["code_block"]["content"]["color"] = (
            _to_hex(code_font.color.rgb) or config["code_block"]["content"]["color"]
        )
    ...
    if quote_style is not None:
        ...
        config["quote"]["font_size"] = (
            _pt(quote_font.size) or config["quote"]["font_size"]
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--template",
        default=None,
        help="模板文件路径（.docx），默认使用 assets/templates/ 目录下第一个 .docx 文件",
    )
