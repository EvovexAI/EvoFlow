...
def insert_image_to_word(doc, image):
    ...
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as temp_file:
        ...
        image = _postprocess_image_for_word(
            image, target_display_cm, target_dpi=target_dpi
        )
        try:
    ...
    try:
        ...
        if "1" in page_format:
            ...
            instr_text = parse_xml(
                r'<w:instrText xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"> PAGE </w:instrText>'
            )
        ...
        if "x" in page_format:
            ...
            instr_text2 = parse_xml(
                r'<w:instrText xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"> NUMPAGES </w:instrText>'
            )
...
def find_template_file():
    ...
    for docx_file in docx_files:
        ...
        if not any(
            keyword in filename for keyword in ["完整版", "test", "output", "输出"]
        ):
...
def debug_quotes_in_file(file_path):
    ...
    print(
        f"📊 引号统计: ASCII双引号={ascii_double}, 中文开引号={chinese_open}, 中文闭引号={chinese_close}"
    )
...
def create_word_document(
    md_file_path, output_path, template_file=None, config: Config = None
):
    ...
    try:
        ...
        normal_style._element.rPr.rFonts.set(
            qn("w:ascii"), font_config.get("ascii", "Times New Roman")
        )
        normal_style._element.rPr.rFonts.set(
            qn("w:hAnsi"), font_config.get("ascii", "Times New Roman")
        )
        normal_style._element.rPr.rFonts.set(
            qn("w:eastAsia"), font_config.get("name", "仿宋_GB2312")
        )
        normal_style._element.rPr.rFonts.set(
            qn("w:cs"), font_config.get("ascii", "Times New Roman")
        )
...
示例:
  ...
  %(prog)s --list-presets
    ...
    parser.add_argument(
        "--list-presets", action="store_true", help="列出所有可用的预设配置"
    )
...
def print_success_info(filename=None, config: Config = None):
    ...
    page_config = config.get("page", {})
    print(
        f"📄 页面大小: {page_config.get('width', 21.0)}cm × {page_config.get('height', 29.7)}cm"
    )
    print(
        f"📐 页边距: 上下{page_config.get('margin_top', 2.54)}cm，左右{page_config.get('margin_left', 3.18)}cm"
    )
    ...
    print(
        f"🎯 一级标题: {title1_config.get('size', 15)}pt，{'加粗' if title1_config.get('bold') else '常规'}"
    )
