        ...
        for part_text, formats in text_parts:
            if part_text:  # 只有非空文本才创建run
                ...
                set_run_format_with_styles(
                    run, formats, title_level=title_level, is_quote=is_quote
                )
...
def set_run_format(run, title_level=0):
    ...
    # 根据标题级别设置字号、加粗、字体和颜色
    if title_level >= 1:
        title_config = config.get(
            f"titles.level{title_level}", config.get("titles.level1", {})
        )
...
def set_paragraph_format(paragraph, title_level=0, is_quote=False):
    ...
    if title_level == 1:
        ...
        paragraph_format.first_line_indent = Pt(
            paragraph_config.get("first_line_indent", 24)
        )
