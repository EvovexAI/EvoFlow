...
def create_word_table(doc, table_lines):
    ...
    table.alignment = alignment_map.get(
        alignment_str.lower(), WD_TABLE_ALIGNMENT.CENTER
    )
    ...
    vertical_align = vertical_align_map.get(
        vertical_align_str.lower(), WD_ALIGN_VERTICAL.CENTER
    )
        ...
        if i + 1 < len(table.rows):
            ...
            for j, cell_text in enumerate(row_data):
                if j < len(row_cells):
                    ...
                    # 处理表格单元格中的格式
                    if contains_markdown_formatting(cell_text.strip()):
                        parse_table_cell_formatting(
                            cell, cell_text.strip(), is_header=False
                        )
                    else:
                        # 导入 convert_quotes_to_chinese 避免循环导入
                        from formatter import convert_quotes_to_chinese
