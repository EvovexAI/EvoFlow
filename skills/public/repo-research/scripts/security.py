...
class SecurityAnalyzer:
    ...
    def _analyze_skill_structure(self) -> Dict:
        ...
        if scripts_dir.exists() and scripts_dir.is_dir():
            ...
            script_files = list(scripts_dir.glob("*.py")) + list(
                scripts_dir.glob("*.js")
            )
    ...
    def _analyze_prompt_security(self) -> Dict:
        ...
        # 过滤排除目录
        md_files = [
            f for f in md_files if not any(exc in f.parts for exc in self.exclude_dirs)
        ]
    ...
    def generate_report(self) -> str:
        ...
        if skill_info["is_skill"]:
            ...
            lines.append("")
            lines.append(
                f"- **SKILL.md**: {'✅ 存在' if skill_info['has_skill_md'] else '❌ 缺失'}"
            )
            lines.append(
                f"- **脚本目录**: {'✅ 存在' if skill_info['has_scripts'] else '❌ 缺失'}"
            )
        ...
        if analysis["findings"]:
            ...
            for severity in ["critical", "high", "medium", "low"]:
                findings = [
                    f for f in analysis["findings"] if f["severity"] == severity
                ]
                if not findings:
                    continue
